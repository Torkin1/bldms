...
static struct bldms_device *device;
...
__SYSCALL_DEFINEx(1, _invalidate_data, int, offset){
    ...
    bldms_abort_op_if_device_unavailable(device);
    
    res = bldms_start_op_on_block(device, offset);
    if (res < 0){
    ...
    if (!bldms_block_contains_valid_data(device, offset)){
    ...
    // release block
    res = bldms_invalidate_block(device, offset);
    if (res < 0){
...
invalidate_data_exit:
    bldms_end_op_on_block(device, offset);
...
__SYSCALL_DEFINEx(3, _get_data, int, offset, char *, destination, size_t, size){
    ...
    block = bldms_block_alloc(device->block_size);
    block->header.index = offset;

    // copy data from block to destination
    res = bldms_move_block(device, block, REQ_OP_READ);
    if (res < 0){
    ...
    /**
     * TODO: userspace needs to give a size big enough to hold an entire block
     * capacity in order to avoid getting truncated data from the block. This is
     * challenging because the block capacity is known at runtime after 
     * calculating the block header size. 
     * Maybe write the block capacity info in a sysfs pseudo file?
     * Maybe allocate a compile-time known header size, filling the unused space
     * with zeroes?
    */
...
__SYSCALL_DEFINEx(2, _put_data, char *, source, size_t, size){
    ...
    block_index = bldms_prepare_write_on_block_any(device);
    if (block_index < 0){
        pr_err("%s: failed to reserve block\n", __func__);
        return -ENOMEM;
    }

    res = bldms_start_op_on_block(device, block_index);
    if (res < 0){
        pr_err("%s: failed to start op on block %d\n", __func__, block_index);
        bldms_undo_write_on_block(device, block_index);
        return -1;
    }
    ...
    if(copied_size != size){
        ...
        bldms_invalidate_block(device, block_index);
        block_index = -1;
        goto put_data_exit;
    }

    // write block to device
    res = bldms_move_block(device, block, REQ_OP_WRITE);
    if (res < 0){
...
put_data_exit:
    bldms_commit_write_on_block(device, block_index);
    bldms_end_op_on_block(device, block_index);
    bldms_block_free(block);
    return block_index;
}
