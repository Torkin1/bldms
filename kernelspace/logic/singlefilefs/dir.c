...
static int onefilefs_iterate(struct file *file, struct dir_context* ctx) {
	...
	if (ctx->pos == 0){
		if(!dir_emit(ctx,".", SINGLEFILEFS_FILENAME_MAXLEN, SINGLEFILEFS_ROOT_INODE_NUMBER, DT_UNKNOWN)){
			return 0;
		}
		else{
			ctx->pos++;
	...
	if (ctx->pos == 1){
		...
		if(!dir_emit(ctx,"..", SINGLEFILEFS_FILENAME_MAXLEN, 1, DT_UNKNOWN)){
			return 0;
		}
		else{
			ctx->pos++;
	...
	if (ctx->pos == 2){
		if(!dir_emit(ctx, SINGLEFILEFS_UNIQUE_FILE_NAME, SINGLEFILEFS_FILENAME_MAXLEN, SINGLEFILEFS_FILE_INODE_NUMBER, DT_UNKNOWN)){
			return 0;
		}
		else{
			ctx->pos++;
...
const struct file_operations singlefilefs_dir_operations = {
    .owner = THIS_MODULE,
